...
class PrepareData:
    ...
    def renameJPG(self, filePath, kind):
        ...
        for name in images:
            if (name.split('_')[0] in ['0', '1', '2', '3','4']):
                continue
            else:
...
def main_args():
    ...
    parser.add_argument('--src_images_dir', type=str, default='./source_image/sodacan',
                        help="训练集的源图片路径")
    parser.add_argument("--split_rate", type=int, default=0.9, help='将训练集二和测试集划分的比例，0.9表示训练集与占90%')
    parser.add_argument('--out_img_size', type=tuple, default=(249, 249),
                        help='保存图片的大小，如果使用简单网络结构参数大小为(100,100),如果使用resnet大小参数为(224,224)')
    parser.add_argument("--save_dir", type=str, default='./sodacan', help='训练数据的保存位置')
